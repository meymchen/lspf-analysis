 ...
 */
test('a real stdio server reports file health and hovers a function', { timeout: 15000 }, async t => {
    ...
    const connection = createMessageConnection(
        new StreamMessageReader(server.stdout), new StreamMessageWriter(server.stdin),
    );
    t.after(() => connection.dispose());
    ...
    const text = 'fn wide(a: u32, b: u32, c: u32, d: u32, e: u32) -> u32 {\n    a + b + c + d + e\n}\n';
    ...
    const hover = await connection.sendRequest<{ contents: { value: string } } | null>(
        'textDocument/hover', { textDocument: { uri }, position: { line: 0, character: 4 } },
    );
