...
import {
    DEBUG_PORT_VARIABLE, DEBUG_TRANSPORT_VARIABLE, connectToServer,
    connectToWebSocketServer, debugServerPort, debugServerTransport,
} from '../src/debug.js';
...
test('debug transport defaults to TCP and accepts WebSocket', () => {
    ...
    assert.throws(() => debugServerTransport({ [DEBUG_TRANSPORT_VARIABLE]: 'udp' }), /must be tcp or ws/);
});
...
test('legacy TCP port works and the generic port takes precedence', () => {
    ...
    assert.equal(debugServerPort({ LSPF_ANALYSIS_DEBUG_TCP: '9257', [DEBUG_PORT_VARIABLE]: '9258' }), 9258);
});
...
test('WebSocket retries startup, exchanges JSON text frames, and closes on disposal', { timeout: 5000 }, async t => {
    const port = await freePort();
    ...
    t.after(() => { transport.reader.dispose(); transport.writer.dispose(); });
...
test('WebSocket failure names the required server mode', { timeout: 5000 }, async () => {
    const port = await freePort();
    await assert.rejects(connectToWebSocketServer(port, { timeout: 150, interval: 25 }), /serve --ws/);
});

test('WebSocket handshake timeout closes a peer that never upgrades', { timeout: 5000 }, async t => {
