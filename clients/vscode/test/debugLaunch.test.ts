...
test('the debug menu exposes exactly stdio, tcp, and websocket without prompts', () => {
  ...
  assert.deepEqual(
    visible.map((config) => config.name),
    ['stdio', 'tcp', 'websocket'],
  );
  ...
  for (const config of visible) {
    ...
    assert.deepEqual(prepare.dependsOn, [
      ...
      'prepare extension development host',
    ]);
  }
});
...
test('stdio host clears inherited network debug settings', () => {
  ...
  for (const variable of [
    'LSPF_ANALYSIS_DEBUG_PORT',
    'LSPF_ANALYSIS_DEBUG_TCP',
    'LSPF_ANALYSIS_DEBUG_TRANSPORT',
    'LSPF_ANALYSIS_DEBUG_LOG',
  ]) {
    assert.equal(host.env[variable], null);
  }
});

for (const [name, protocol, port] of [
  ['tcp', 'tcp', '9257'],
  ['websocket', 'ws', '9258'],
]) {
  test(`${name} launches matching server and host processes after preparation`, () => {
    ...
    for (const part of parts) {
      ...
      assert.equal(
        part.preLaunchTask,
        undefined,
        'build both parts before starting either process',
      );
    }
  });
}
