...
test(
  'a configured POSIX path wins over both other sources',
...
test(
  'a configured Windows path wins over both other sources',
  { skip: process.platform !== 'win32' },
  () => {
    for (const development of [true, false]) {
      assert.deepEqual(
...
test('the missing-server message names the path and the likely cause', () => {
  ...
  assert.match(describeMissingServer({ binary: '/nope', source: 'development' }), /cargo build/);
});
