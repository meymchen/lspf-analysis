...
macro_rules! mk_action {
    ( $( ($camel:ident, $parser:ident) ),* ) => {
        ...
        ///
        /// use lspf_analysis_core::{action, Callback, LANG, Metrics, MetricsCfg};
        ///
        ...
        /// };
        ///
        /// action::<Metrics>(&language, source_as_vec, &cfg.path.clone(), cfg);
        /// ```
        ///
        /// [`Callback`]: trait.Callback.html
        #[inline(always)]
        pub fn action<T: Callback>(lang: &LANG, source: Vec<u8>, path: &Path, cfg: T::Cfg) -> T::Res {
            match lang {
                $(
                    LANG::$camel => {
                        let parser = $parser::new(source, path);
                        T::call(cfg, &parser)
                    },
                )*
            }
        ...
        /// use std::path::PathBuf;
        ///
        /// use lspf_analysis_core::{get_function_spaces, LANG};
        ///
        /// let source_code = "fn main() {}";
        /// let language = LANG::Rust;
        ///
...
macro_rules! mk_extensions {
    ( $( ($camel:ident, [ $( $ext:ident ),* ]) ),* ) => {
        ...
        ///
        /// ```
        /// use lspf_analysis_core::get_from_ext;
        ///
        /// let ext = "rs";
        ///
...
macro_rules! mk_emacs_mode {
    ( $( ($camel:ident, [ $( $emacs_mode:expr ),* ]) ),* ) => {
        ...
        ///
        /// ```
        /// use lspf_analysis_core::get_from_emacs_mode;
        ///
        /// let emacs_mode = "rust";
        ///
