...
///
/// use lspf_analysis_core::{RustParser, dump_root, metrics, ParserTrait};
///
...
fn dump_metrics(
    ...
    dump_working_memory(&metrics.working_memory, &prefix, true, stdout)
}
...
fn dump_working_memory(
    ...
    intense_color(stdout, Color::Green)?;
    write!(stdout, "working_memory: ")?;

    color(stdout, Color::White)?;
    writeln!(stdout, "{}", stats.wm())
}
