...
pub struct Stats {
    ...
    conditions_max: f64,
    space_count: usize,
    /// The declaration being read, innermost last.
    ///
    /// A Java assignment only counts when it initializes a variable rather
    /// than a constant, which is not decidable from the `=` node alone: the
    /// walk has to remember what declaration it is inside.
...
/// What a declaration binds: something that can be assigned again, or
