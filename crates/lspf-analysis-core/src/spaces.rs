...
pub struct CodeMetrics {
    ...
    pub npa: npa::Stats,
    /// `WorkingMemory` data
    pub working_memory: working_memory::Stats,
}
...
fn compute_averages(state: &mut State) {
    ...
    state.space.metrics.working_memory.finalize(nom_total);
}
...
fn compute_minmax(state: &mut State) {
    ...
    state.space.metrics.working_memory.compute_minmax();
}
...
/// use std::path::Path;
///
/// use lspf_analysis_core::{RustParser, metrics, ParserTrait};
///
...
/// let parser = RustParser::new(source_as_vec, &path);
///
/// // Gets all function spaces data of the code contained in foo.rs
...
pub fn metrics<'a, T: ParserTrait>(parser: &'a T, path: &'a Path) -> Option<FuncSpace> {
    ...
    while let Some((node, level)) = stack.pop() {
        ...
        let new_level = if func_space {
            ...
            // Working memory needs the whole function body at once, so it is
            // computed here, where the space node is still in scope, rather
            // than one node at a time like the other metrics.
            if let Some(state) = state_stack.last_mut() {
                T::WorkingMemory::compute_space(
                    &node,
                    code,
                    &mut state.space.metrics.working_memory,
                );
            }
            last_level = level + 1;
            last_level
